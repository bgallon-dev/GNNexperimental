r"""Adversarial verification of stress_robustness (key=robustness).

Identical measurement to scripts/stress_robustness.py but on a DISJOINT slice
(graphs [start:start+n]) and a different seed, to test whether the FRAGILE
verdict replicates. Mirrors the reference idioms exactly.

Run from kettle-graph-reasoner/:
    py -m scripts.stress_robustness_verify --start 100 --n 60 --seed 13 \
        --out runs/stress_robustness_verify
"""
...
def main() -> None:
    ...
    ap.add_argument("--out", default="runs/stress_robustness_verify")
