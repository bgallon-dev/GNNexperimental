r"""Adversarial re-run of stress_boundary_mask on a DISJOINT slice.

Same measurement as scripts/stress_boundary_mask.py but on graphs
[start .. start+n) (default 100..160, disjoint from the original 0..60)
and a different RNG seed for the size-matched random control.

The original headline (key=boundary_mask): a distance-thresholded subgraph
boundary mask gives global F1 ~= size-matched random F1 (no capability):
  global_f1 0.384 ~ random_sizematched_f1 0.385 ; multihop 0.818 outlier.

Refute test: does that null (model F1 ~= random F1) replicate on 100..160?

Run from kettle-graph-reasoner/:
    py -m scripts.stress_boundary_mask_refute \
        --corpus src/data/corpus/real_domain_eval_all6 \
        --start-graph 100 --n-graphs 60 --seed 7 \
        --out runs/stress_boundary_mask_refute
"""
...
def main() -> None:
    ...
    ap.add_argument("--start-graph", type=int, default=100)
    ap.add_argument("--n-graphs", type=int, default=60)
    ap.add_argument("--rand-samples", type=int, default=20)
    ap.add_argument("--seed", type=int, default=7)
    ap.add_argument("--out", default="runs/stress_boundary_mask_refute")
    ...
    files = all_files[args.start_graph: args.start_graph + args.n_graphs]
    print(f"{len(files)} graphs (index {args.start_graph}.."
          f"{args.start_graph + len(files)})")
    for f in files:
