...
2. **Learnable `tangent_scale` parameter at the input projection only** (`KettleGraphReasoner.tangent_scale`, init 0.5). Multiplies the tangent vector before the first `expmap0`. With small-gain init producing tangent norms ~1.0, scale 0.5 lands initial points at ||h|| ≈ tanh(0.5) ≈ 0.46 — inside the regime where hyperbolic curvature is active but not saturated. Earlier values 0.1 (too conservative — crushed to near-origin shell) and 0.5 stacked with a *second* `tangent_scale` inside the MP layer's HNN nonlinearity both failed: stacking three independent damping factors per layer (small-gain `weight`, input `tangent_scale`, MP `tangent_scale`) erased per-node radial variance. The MP layer's nonlinearity no longer rescales; scale is controlled only by the weight init and the input `tangent_scale`.
