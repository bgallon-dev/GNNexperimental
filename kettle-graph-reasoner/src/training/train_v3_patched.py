...
def _stage_a(cfg: Config, model, dataset: CorpusDataset, device) -> list[dict]:
    ...
    for epoch in range(cfg.contrastive_epochs):
        ...
        for batch_idx in order:
            ...
            history.append({"step": step, "epoch": epoch, **diag, "reg_w": reg_w})
            step += 1
...
def train(cfg: Config) -> dict:
    ...
    # Graph-level train filter for 2.2 transfer experiment.
    # Deterministically hold out (1 - train_graphs_frac) of unique train
    # graphs. The held-out graphs become "unseen_train" in eval.
    all_train_graph_ids = sorted({int(gi) for gi, _ in train_ds.index})
    if cfg.train_graphs_frac < 1.0:
        rng_split = np.random.default_rng(cfg.seed + 7919)
        n_total = len(all_train_graph_ids)
        n_keep = max(1, int(round(cfg.train_graphs_frac * n_total)))
        perm = rng_split.permutation(n_total)
        kept_ids = sorted(all_train_graph_ids[i] for i in perm[:n_keep])
        held_ids = sorted(all_train_graph_ids[i] for i in perm[n_keep:])
        kept_set = set(kept_ids)
        # In-place filter of dataset.index so both stages operate on
        # only the kept graphs.
        train_ds.index = [
            (gi, ti) for (gi, ti) in train_ds.index if int(gi) in kept_set
        ]
        print(f"[train_v3] graph-level train filter: "
              f"{len(kept_ids)}/{n_total} graphs kept "
              f"(train_graphs_frac={cfg.train_graphs_frac})")
        print(f"[train_v3] held out graph_ids: {held_ids[:20]}"
              + (" ..." if len(held_ids) > 20 else ""))
    else:
        kept_ids = all_train_graph_ids
        held_ids = []
    ...
    summary = {
        ...
        "training_graph_ids": kept_ids,
        "held_out_graph_ids": held_ids,
        "config": asdict(cfg),
    }
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--train-graphs-frac", type=float, default=1.0,
                   help="Fraction of unique train GRAPHS used (affects both "
                        "stage A and stage B). 1.0 = use all. <1.0 holds "
                        "out the complement for 2.2 transfer evaluation.")
