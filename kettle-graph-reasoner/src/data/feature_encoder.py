...
Query features: 17 dimensions (task one-hot + temporal/hops + anchor identity)
...
QUERY_TASK_DIM = 6  # task flags (slots 0-4 are the 5 base types,
# slot 5 is the compound marker). Multi-hot
# for compound tasks: compound bit + each
# component bit are set simultaneously.
QUERY_TEMPORAL_DIM = 2  # start, end
QUERY_HOPS_DIM = 1  # max hops (normalized)
QUERY_PAD_DIM = 1  # padding slot (unused)
QUERY_ANCHOR_DIM = 8  # anchor identity vector (Task 1 only, else zeros)


def encode_nodes(graph: SyntheticGraph) -> tuple[np.ndarray, Dict[int, int]]:
    """
    ...
    for nid in sorted(graph.nodes.keys()):
        ...
        # Normalize by BuilderConfig.max_depth (5), not 10 — the old divisor
        # left the top half of this feature's range permanently unused.
        features[row, struct_offset + 4] = nd.depth / 5.0  # normalized depth
...
def encode_query(
    ...
    Args:
        task_type: 0-4 for a single-type task, 5 for a compound task.
        ...
        anchor_features: full (NODE_FEAT_DIM,) feature row for the anchor.
            Used to extract the 8-dim identity vector (dims 24:32) into the
            query. Required for Task 1 (ER); also applied to compound tasks
            whose component set includes ER.
        component_tasks: tuple of component task ids for compound tasks;
            ignored when task_type != 5.

    Returns:
        (QUERY_FEAT_DIM,) feature vector

    Feature layout:
        [0:6]    - task flags (slots 0-4 base types, slot 5 compound marker;
                   multi-hot for compound tasks)
        [6:8]    - temporal window (start, end)
        [8]      - max hops (normalized by 10)
        [9]      - padding
        [10:18]  - anchor identity vector (ER or ER-compound; zero otherwise)
    """
    q = np.zeros(QUERY_FEAT_DIM, dtype=np.float32)

    # Task flags — multi-hot for compound tasks.
    ...
    if temporal_window is not None:
        q[6] = temporal_window[0]
        q[7] = temporal_window[1]

    # Max hops (normalized)
    q[8] = max_hops / 10.0

    # Anchor identity for ER (and any compound that includes ER).
    needs_identity = anchor_features is not None
    if needs_identity and anchor_features is not None:
...
if __name__ == "__main__":
    ...
    # Encode query
    query = encode_query(
        task_type=0, anchor_row=5, temporal_window=(0.2, 0.8), max_hops=3
    )
